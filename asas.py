...
def kerdojel(tarsas1, visszafele, ajtok):
    os.system('cls')
    stats.lebukas += 4
    stats.energia -= 2
    stats.line()
...
def sarok(energia, stats, egeszseg, vodor):
    os.system('cls')
    stats.lebukas += 4
    stats.energia -= 2
    stats.line()
    ...
    if folyoso == 1:
        ...
        vodor += 1
        print('\n')
        print('Elmész a nyél, és szögek után is kutatni, vagy inkább mára abbahagyod a császkálást?')
        print('\t 1 - Folytatom')
...
def valasztott_2(nev, valasztott1, sarok, egeszseg):
        if valasztott1 == 2:
            ...
            if terv2 == 1:
                sarok(energia, stats, egeszseg, vodor)
            else:
                os.system('cls')
